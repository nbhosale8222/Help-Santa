...
const MapMainView = () => {
  ...
  // Show victory video when returning to map after completing level 10
  useEffect(() => {
    // Check if level 10 was just completed
    const hasCompletedLevel10 = gameState.progress.includes(10);
    const justCompletedLevel10 = gameState.lastCompletedLevel === 10;
    
    if (hasCompletedLevel10 && justCompletedLevel10 && !showVictoryVideo) {
      setShowVictoryVideo(true);
    }
  }, [gameState.progress, gameState.lastCompletedLevel, showVictoryVideo]);
  ...
  const handleLevelComplete = (completedLevelId) => {
    ...
    // Show victory video after level 10
  ...
  // Use longer timeout for level 4 and 8 (special cutscene videos), shorter for others
  const transitionDuration = (gameState.currentLevel === 4 || gameState.currentLevel === 8) ? 30000 : 3000;
                ...
                <video
                  src={
                    gameState.currentLevel === 4 
                      ? "/Dark_Christmas_Fantasy_Cutscene_Generated.mp4" 
                      : gameState.currentLevel === 8
                      ? "/Christmas_Fantasy_Adventure_Cutscene_Generated.mp4"
                      : "/transition.webm"
                  }
                  autoPlay
                  muted
                  ...
                  onEnded={() => navigate(`/level-reading/${gameState.currentLevel}`)}
                />
              </div>
            )}
              ...
              <div className="fixed inset-0 z-50 bg-black flex items-center ">
                <video
                  ...
                  onEnded={() => {
                    setShowVictoryVideo(false);
                    dispatch(updateState({
                      progress: [...gameState.progress, 10],
                      lastCompletedLevel: 10,
                    }));
                  }}
                />
              </div>
